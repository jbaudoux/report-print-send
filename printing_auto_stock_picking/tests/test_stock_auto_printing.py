...
class TestAutoPrinting(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.auto_printing_report = cls.env["printing.auto"].create(
            {
        ...
        cls.auto_printing_attachment = cls.env["printing.auto"].create(
            {
        ...
        cls.stock_picking.picking_type_id.auto_printing_ids |= (
            cls.auto_printing_report | cls.auto_printing_attachment
        )
    ...
    def test_10_test_print_report(self):
        self.auto_printing_report.do_print(self.stock_picking)

    def test_20_test_print_attachment(self):
        self.auto_printing_attachment.do_print(self.stock_picking)

    def test_30_test_condition_filter(self):
        self.auto_printing_report.condition = [("id", "=", self.stock_picking.id)]
        auto_printing = self.auto_printing_report._check_condition(self.stock_picking)
        self.assertEqual(auto_printing, self.stock_picking)

        self.auto_printing_report.condition = [("id", "=", 0)]
        auto_printing = self.auto_printing_report._check_condition(self.stock_picking)
    ...
    def test_40_test_attachment_filter(self):
        ...
        self.auto_printing_attachment.attachment_domain = [("id", "=", attachment_2.id)]
        test_attachment = self.auto_printing_attachment.generate_data_from_attachments(
            self.stock_picking
        )
    ...
    def test_50_test_field_object(self):
        self.auto_printing_report.field_object = "picking_type_id"
        field_object = self.auto_printing_report._get_record(self.stock_picking)
        self.assertEqual(field_object, self.stock_picking.picking_type_id)

    def test_50_test_printer_and_behaviour(self):
        behaviour = self.auto_printing_report.get_behaviour()
        self.assertEqual(behaviour["printer"], self.auto_printing_report.printer_id)
        self.assertFalse(behaviour["tray"])

        self.auto_printing_report.printer_tray_id = self.tray
        behaviour = self.auto_printing_report.get_behaviour()
        self.assertEqual(behaviour["printer"], self.auto_printing_report.printer_id)
        self.assertEqual(behaviour["tray"], self.tray.system_name)

        self.auto_printing_report.printer_tray_id = None
        self.auto_printing_report.printer_id = None
    ...
    def test_60_output(self):
        # Report
        for content in self.auto_printing_report._get_content(self.stock_picking):
        ...
        self.auto_printing_attachment.attachment_domain = [
            ("name", "in", attachment_names)
        ]
        contents = self.auto_printing_attachment._get_content(self.stock_picking)
